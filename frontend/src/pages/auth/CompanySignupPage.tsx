...
export default function CompanySignupPage(): JSX.Element {
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-sm text-[var(--muted)]">Post opportunities and discover student talent.</p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-4">
        ...
        <div className="space-y-2 rounded-xl border border-black/40 dark:border-white/40 bg-[#DEDEDE]/60 dark:bg-[#030303]/60 p-3">
          <p className="text-xs font-medium uppercase tracking-[0.12em] text-[var(--muted)]">
            Verification documents (PDF only)
          </p>
          <p className="text-xs text-[var(--foreground)]">Upload 1-5 files, maximum 10MB each.</p>
          ...
          <input
            ...
            className="block w-full text-xs text-[var(--muted)]/10 file:mr-3 file:rounded-full file:border file:border-black/20 file:dark:border-white/20 file:bg-black/5 file:dark:bg-white/5 file:px-3 file:py-1.5 file:text-xs file:text-[var(--muted)] hover:file:bg-black/10 hover:file:dark:bg-white/10"
          />

          {documents.length > 0 && (
            <ul className="space-y-1.5 text-xs text-[var(--muted)]/10">
              {documents.map((doc, index) => (
                <li key={`${doc.name}-${doc.lastModified}`} className="flex items-center justify-between gap-2 rounded bg-black/5 dark:bg-white/5 px-2 py-1.5">
                  <span className="truncate">
                    {doc.name} ({Math.ceil(doc.size / 1024)} KB)
                  </span>
                  <button
                    type="button"
                    className="text-[var(--muted)] underline underline-offset-2 hover:text-black dark:hover:text-white"

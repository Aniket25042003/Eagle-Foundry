  ...
  return (
    <div className="space-y-8">
        ...
        <Card>
          <div className="flex flex-wrap items-start gap-6">
            ...
            <div className="flex-1 space-y-2">
              ...
              {profile.major && (
                <p className="text-sm text-[var(--muted)]">{profile.major}</p>
              )}
      ...
      {activeTab === 'Application Form' && (
        <Card>
          <h3 className="mb-4 text-sm font-medium uppercase tracking-[0.12em] text-[var(--muted)]">
            Application Details
          </h3>
          {!application.formAnswers || Object.keys(application.formAnswers).length === 0 ? (
            <p className="text-sm text-[var(--muted)]">No application form data provided.</p>
          ) : (
            <dl className="space-y-4 text-sm">
              {application.formAnswers.firstName && (
                <div>
                  <dt className="text-[var(--muted)]">First Name</dt>
                  <dd className="text-[var(--foreground)] mt-1">{application.formAnswers.firstName}</dd>
                </div>
              )}
              {application.formAnswers.lastName && (
                <div>
                  <dt className="text-[var(--muted)]">Last Name</dt>
                  <dd className="text-[var(--foreground)] mt-1">{application.formAnswers.lastName}</dd>
                </div>
              )}
              {application.formAnswers.address && (
                <div>
                  <dt className="text-[var(--muted)]">Address</dt>
                  <dd className="text-[var(--foreground)] mt-1">{application.formAnswers.address}</dd>
                </div>
              )}
              ...
              {application.formAnswers.customAnswers &&
                ...
                Object.keys(application.formAnswers.customAnswers).length > 0 && (
                  <div className="pt-4 border-t border-[var(--border)] mt-4">
                    <h4 className="text-xs font-semibold uppercase tracking-wider text-[var(--muted)] mb-3">Custom Questions</h4>
                    <div className="space-y-4">
                      {Object.entries(application.formAnswers.customAnswers).map(([qId, answer]) => (
                        <div key={qId}>
                          <dt className="text-[var(--muted)] text-xs mb-1">Answer to custom question:</dt>
                          <dd className="text-[var(--foreground)] bg-[var(--elements)] p-3 rounded-lg whitespace-pre-wrap border border-[var(--border)]">
                            {String(answer)}
                          </dd>
                        </div>
                      ))}
      ...
      {activeTab === 'Cover Letter' && (
        <Card>
          <h3 className="mb-3 text-sm font-medium uppercase tracking-[0.12em] text-[var(--muted)]">
            Cover Letter
          </h3>
          ...
          {application.resumeUrl && (
            <div className="mt-4">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="text-sm text-[var(--foreground)] underline hover:text-[var(--foreground)]"
              >
                Download resume
              </a>
            </div>
            ...
            {statusHistory.length === 0 ? (
              ...
              statusHistory.map((entry: ApplicationStatusHistoryEntry) => (
                ...
                >
                  ...
                  {entry.note && (
                    <p className="w-full text-sm text-[var(--muted)]">{entry.note}</p>
                  )}
                </div>
              ))
            )}

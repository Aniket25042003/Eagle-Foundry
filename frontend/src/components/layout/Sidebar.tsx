...
function NavGroup({ label, items }: { label: string; items: NavItem[] }): JSX.Element {
  return (
    <div className="mb-5">
      <p className="mb-2 px-3 text-[10px] font-semibold uppercase tracking-[0.16em] text-[var(--muted)]">{label}</p>
            ...
            className={({ isActive }) =>
              cn(
                ...
                isActive
                  ...
                  : 'text-[var(--muted)] hover:bg-[var(--elements)] hover:text-[var(--foreground)]',
              )
            }
          >
            <item.icon size={16} />
...
export function Sidebar({ collapsed }: SidebarProps): JSX.Element {
  ...
  return (
    <motion.aside
      className="flex h-full shrink-0 flex-col overflow-hidden border-r border-[var(--border)] bg-[var(--background)] px-2 py-4 backdrop-blur-md"
      initial={false}
